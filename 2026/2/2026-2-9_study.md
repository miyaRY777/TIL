...
**`association :user` は、「このモデルは user に必ず紐づく」とFactoryBotに教える指定。**  
テスト用データを作るときに、関連する user を自動で用意してくれる。

---

## 理由

Railsでは、モデル同士のつながり（関連）をとても重要に扱う。  
特に `belongs_to :user` があるモデルは、user が無いと保存できない。  
`association :user` を書くことで、FactoryBotが必要な user を自動生成してくれる。

---

## 具体例

Profileモデルが次のような関連を持っている場合。
...
`create(:profile)` を実行すると、

- user が自動で作られる
- その user に紐づいた profile が作られる

自分で `user = create(:user)` と書く必要はない。

---

## 結論

**`association :user` は、「関連モデルも一緒に作ってね」という指示。**  
belongs_to があるFactoryでは、ほぼ必須の書き方。
